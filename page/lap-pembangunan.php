...
while ($li = mysqli_fetch_array($ress_kas)) {
    ...
    $col['pemasukan'] = $li['pembangunan'];
    $col['pengeluaran'] = $li['pengeluaran_pembangunan'];
    $data[] = $col;
}

// data infaq (operasional)
$sql_infaq = "SELECT a.*, b.nama_donatur FROM tr_sodaqoh a, m_donatur b WHERE a.id_petugas = '".$_SESSION['id_petugas']."' AND b.id_donatur = a.id_donatur ORDER BY a.tanggal ASC";
...
// data pengeluaran
$sql_keluar = "SELECT * FROM tr_pengeluaran  WHERE id_petugas = '".$_SESSION['id_petugas']."' AND jenis_pengeluaran = 'PEMBANGUNAN' ORDER BY tanggal ASC";
                ...
                <div class="row clearfix">
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                        <form class="form-inline" action="" method="POST">
                            ...
                            <input type="hidden" name="pembangunan" value="<?php echo $sum_sodaqoh ?>">
                            ...
                            <?php
                            if ($i == 1) {
                                ?> <a href="./page/lap-pembangunan-cetak.php?masjid=<?php echo $_SESSION["id_masjid"] ?>&petugas=<?php echo $_SESSION["id_petugas"] ?>&bln=<?php echo $param ?>&thn=<?php echo $param2 ?>" target="_blank" class="btn btn-primary" disabled>Cetak</a>
                            <?php } else { ?>
                                <a href="./page/lap-pembangunan-cetak.php?masjid=<?php echo $_SESSION["id_masjid"] ?>&petugas=<?php echo $_SESSION["id_petugas"] ?>&bln=<?php echo $param ?>&thn=<?php echo $param2 ?>" target=" _blank" class="btn btn-primary">Cetak</a>
                            <?php }
                            ?>
                        </form>
                    </div>
...
if (isset($_POST['rekap'])) {
    ...
    if ($rows == 1) {
        ...
        $sql = "UPDATE tr_rekapkas SET
				...
				pengeluaran_pembangunan='" . $pengeluaran . "',
                id_masjid='" . $id_masjid . "'
				WHERE no_rekap='" . $li['no_rekap'] . "'";
        $ress = mysqli_query($kdb, $sql);
        if ($ress) {
            echo "<script>alert('Data berhasil disimpan!'); window.location='index.php?page=lap-pembangunan';</script>";
        }
        ...
        $sql = "INSERT INTO tr_rekapkas (no_rekap, tgl_rekap, tgl_tampil, pembangunan, pengeluaran_pembangunan, id_masjid) VALUES (
                '" . $newid_rekap . "',
                '" . $tgl_rekap . "',
                '" . $tgl_tampil . "',
                '" . $pembangunan . "',
