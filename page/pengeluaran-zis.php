...
?>
                            <tr>
                                ...
                                <td>
                                    <a href="?page=pengeluaran-zis&b=edit&pengeluaran=<?php echo $t['id_pengeluaran']; ?>" class="btn btn-success btn-circle waves-effect waves-circle waves-float" title="Edit"> <i class="fa fa-edit"> </i> </a>
...
<div class="modal fade text-xs-left" id="myModalh<?php echo $t['id_pengeluaran']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="?page=pengeluaran-zis" method="post">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <label class="modal-title text-text-bold-600" id="myModalLabel33">Penambahan data pengeluaran kas ZIS </label>
      </div>
      <br>
      <form action="?page=pengeluaran-zis" method="post">
...
<div class="row clearfix">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="header">
                Perubahan data pengeluaran kas ZIS
                <div class="body">
                    <form action="?page=pengeluaran-zis" method="post">
...
function sql_select()
    {
        global $kdb;
        $sql = "SELECT a.*,b.* FROM `tr_pengeluaran` as a, `m_petugas` as b WHERE a.`id_petugas`=b.`id_petugas` and a.jenis_pengeluaran = 'ZIS' and b.`id_masjid`='".$_SESSION['id_masjid']."'";
...
function sql_insert() //Untuk Insert Data
    {
        global $kdb;
        $sql = "INSERT INTO `tr_pengeluaran`(`jenis_pengeluaran`,`tanggal`,`nominal`,`keterangan`,`id_petugas`) VALUES ('ZIS','".$_POST['tanggal']."', '".$_POST['nominal']."','".$_POST['keterangan']."','".$_POST['petugas']."')";
        $hasil = mysqli_query($kdb, $sql) or die(mysqli_error());
        //Peringatan
        if ($hasil){
...
function sql_update()
    {
        global $kdb;
        $sql = "UPDATE `tr_pengeluaran` SET `jenis_pengeluaran` = 'ZIS', `tanggal` = '".$_POST['tanggal']."', `nominal` = '".$_POST['nominal']."', `keterangan` = '".$_POST['keterangan']."', `id_petugas` = '".$_POST['petugas']."' where `id_pengeluaran` = '".$_POST['idpengeluaran']."'";
        $hasil = mysqli_query($kdb, $sql) or die (mysqli_error());
