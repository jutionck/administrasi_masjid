...
while ($li = mysqli_fetch_array($ress_kas)) {
    ...
    $col['sumber'] = "KAS DKM";
    $col['zis'] = $li['zis'];
    $col['pemasukan'] = $li['zis'];
    $col['pengeluaran'] = $li['pengeluaran_zis'];
    $data[] = $col;
}

// data zis (operasional)
$sql_infaq = "SELECT * FROM tr_zis WHERE id_petugas = '".$_SESSION['id_petugas']."' ORDER BY tanggal ASC";
...
// data pengeluaran
$sql_keluar = "SELECT * FROM tr_pengeluaran  WHERE id_petugas = '".$_SESSION['id_petugas']."' AND jenis_pengeluaran = 'ZIS' ORDER BY tanggal ASC";
...
<div class="row clearfix js-sweetalert">
    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
        <div class="card">
            <div class="header">
                <h2>
                    Laporan Kas Dana Zakat Infaq Sodaqoh
                ...
                <div class="row clearfix">
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                        <form class="form-inline" action="" method="POST">
                            ...
                            <?php
                            if ($i == 1) {
                                ?> <a href="./page/lap-zis-cetak.php?masjid=<?php echo $_SESSION["id_masjid"] ?>&petugas=<?php echo $_SESSION["id_petugas"] ?>&bln=<?php echo $param ?>&thn=<?php echo $param2 ?>" target="_blank" class="btn btn-primary" disabled>Cetak</a>
                            <?php } else { ?>
                                <a href="./page/lap-zis-cetak.php?masjid=<?php echo $_SESSION["id_masjid"] ?>&petugas=<?php echo $_SESSION["id_petugas"] ?>&bln=<?php echo $param ?>&thn=<?php echo $param2 ?>" target=" _blank" class="btn btn-primary">Cetak</a>
                            <?php }
                            ?>
                        </form>
                    </div>
...
if (isset($_POST['rekap'])) {
    ...
    if ($rows == 1) {
        ...
        $sql = "UPDATE tr_rekapkas SET
				...
				zis='" . $zis . "',
				pengeluaran_zis='" . $pengeluaran . "',
                id_masjid='" . $id_masjid . "'
				WHERE no_rekap='" . $li['no_rekap'] . "'";
        $ress = mysqli_query($kdb, $sql);
        if ($ress) {
            echo "<script>alert('Data berhasil disimpan!'); window.location='index.php?page=lap-zis';</script>";
        }
        ...
        $sql = "INSERT INTO tr_rekapkas (no_rekap, tgl_rekap, tgl_tampil, zis, pengeluaran_zis, id_masjid) VALUES (
                '" . $newid_rekap . "',
                '" . $tgl_rekap . "',
                '" . $tgl_tampil . "',
                '" . $zis . "',
