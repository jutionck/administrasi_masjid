...
// data sodaqoh
$sql_sodaqoh = "SELECT * FROM tr_sodaqoh INNER JOIN m_donatur ON tr_sodaqoh.id_donatur=m_donatur.id_donatur ORDER BY tanggal ASC";
$ress_sodaqoh = mysqli_query($kdb, $sql_sodaqoh);
...
// data infaq
$sql_infaq = "SELECT * FROM tr_infaq ORDER BY tanggal ASC";
...
// data ZIS
$sql_zis = "SELECT * FROM tr_zis ORDER BY tanggal ASC";
$ress_zis = mysqli_query($kdb, $sql_zis);
while ($li = mysqli_fetch_array($ress_zis)) {
    $col = array();
    $col['tanggal'] = $li['tanggal'];
    $col['uraian'] = $li['keterangan'];
    $col['sumber'] = $li['jenis_zis'];
    $col['operasional'] = 0;
    $col['pembangunan'] = 0;
    $col['zis'] = $li['nominal'];
    $col['pemasukan'] = $li['nominal'];
...
// data pengeluaran
$sql_keluar = "SELECT * FROM tr_pengeluaran ORDER BY tanggal ASC";
                ...
                <div class="row clearfix">
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                        <form class="form-inline" action="" method="POST">
                            ...
                            <input type="hidden" name="zis" value="<?php echo $sum_zis ?>">
                            ...
                            <?php
                            if ($i == 1) {
                                ?> <a href="./page/Kasbulanan_cetak.php?masjid=<?php echo $_SESSION["id_masjid"] ?>&bln=<?php echo $param ?>&thn=<?php echo $param2 ?>" target="_blank" class="btn btn-primary" disabled>Cetak</a>;
                            <?php } else { ?>
                                <a href="./page/Kasbulanan_cetak.php?masjid=<?php echo $_SESSION["id_masjid"] ?>&bln=<?php echo $param ?>&thn=<?php echo $param2 ?>" target=" _blank" class="btn btn-primary">Cetak</a>
                            <?php }
                            ?>
                        </form>
                    </div>
...
if (isset($_POST['rekap'])) {
    ...
    if ($rows == 1) {
        ...
        if ($ress) {
            echo "<script>alert('Data berhasil disimpan!'); window.location='index.php?page=KasBulanan';</script>";
        }
        ...
        $sql = "INSERT INTO tr_rekapkas VALUES (
				'" . $newid_rekap . "',
				'" . $tgl_rekap . "',
				'" . $tgl_tampil . "',
				'" . $operasional . "',
